...
#undef CREATE_MM
    ...
    for (uint32_t w = 0; w < DMMV_WG_SIZE_COUNT; ++w) {
        ...
        for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
            ...
            // Q3_K: pick the wide-batch shader for NUM_COLS>1 (~30% GFLOPS gain at n=4 on Intel Arc).
            // NUM_COLS=1 (tg128 path) stays on the original shader, which is tuned for that case.
            {
                const char *  q3k_name = (i == 0) ? "mul_mat_vec_q3_k_f32_f32"        : "mul_mat_vec_q3_k_wide_f32_f32";
                const size_t  q3k_len  = (i == 0) ? arr_dmmv_q3_k_f32_f32_len[reduc16]  : arr_dmmv_q3_k_wide_f32_f32_len[reduc16];
                const auto *  q3k_data = (i == 0) ? arr_dmmv_q3_k_f32_f32_data[reduc16] : arr_dmmv_q3_k_wide_f32_f32_data[reduc16];
                ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[w][GGML_TYPE_Q3_K][i], q3k_name, q3k_len, q3k_data, "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {wg_size_subgroup16, rm_kq, i+1}, 1, true, use_subgroups16, force_subgroup_size16);
            }
            ...
            {
                const char *  q3k_name = (i == 0) ? "mul_mat_vec_q3_k_f16_f32"        : "mul_mat_vec_q3_k_wide_f16_f32";
                const size_t  q3k_len  = (i == 0) ? arr_dmmv_q3_k_f16_f32_len[reduc16]  : arr_dmmv_q3_k_wide_f16_f32_len[reduc16];
                const auto *  q3k_data = (i == 0) ? arr_dmmv_q3_k_f16_f32_data[reduc16] : arr_dmmv_q3_k_wide_f16_f32_data[reduc16];
                ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[w][GGML_TYPE_Q3_K][i], q3k_name, q3k_len, q3k_data, "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {wg_size_subgroup16, rm_kq, i+1}, 1, true, use_subgroups16, force_subgroup_size16);
            }
...
static bool ggml_vk_khr_cooperative_matrix_support(const vk::PhysicalDeviceProperties& props, const vk::PhysicalDeviceDriverProperties& driver_props, vk_device_architecture arch) {
    ...
    case VK_VENDOR_ID_INTEL:
        // Upstream only allows Xe2; Arc A770-class hardware has perf regressions.
        // Local override GGML_VK_FORCE_INTEL_COOPMAT=1 enables coopmat path on non-Xe2 Intel
        // (e.g. Arrow Lake-H Xe-LPG+) — research escape hatch, may regress.
        if (arch == vk_device_architecture::INTEL_XE2) {
            return true;
        }
        return getenv("GGML_VK_FORCE_INTEL_COOPMAT") != nullptr;
    case VK_VENDOR_ID_AMD:
